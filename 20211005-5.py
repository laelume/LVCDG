...
p3 >> gong([0,4,-7,4,0], pan=PSine(5), dur=1/3, amplify=1)

hh >> play("x % $ [\\\\]", sample=3, amp=var([1, 0], [7, 1, 14, 2]))

hx >> play("[\  \  \  \  ]", amplify=0.03, pan=PSine(7), sample=1,amp=var([1,0],[8,8]))
...
pq >> blip([3,2,3,4,5,4,5,6,5,6,7,8,7,6,5,2,-1],room=20,amplify=0.9,pan=PSine(15)).every(7,'stutter')
...
dr >> jbass([(0,2)], pitch=var([7,5,6,10],1), oct=3, dur=1, amplify=[0.9,0.8,0.6,0.5])

#dr >> varsaw([0], oct=var([3,5],1), dur=1, amplify=0.7)
